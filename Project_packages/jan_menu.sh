...
show_menu
	...
	0) clear;
		...
		pig -x local /home/hduser/pig1b.pig
	...
	5) clear;
        option_picked "3) Which industry has the most number of Data Scientist positions?";
               hadoop jar nyse.jar Qs3 /niit2/h1b_kaggle.csv /niit2/oo1
               hadoop fs -cat /niit2/oo1/p*
       show_menu;
        ;;
	    ...
	    hive -e "use finalproject;  select employer_name,count(*) as total from h1b_final where year=$var group by employer_name order by total desc limit 5;"
         show_menu;
        ;;
	    ...
	    hive -e "use finalproject;  select job_title,count(*) as total from h1b_final where year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	    ...
	    hive -e "use finalproject;  select job_title,count(*) as total from h1b_final where case_status='CERTIFIED' and year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	    ...
	    hive -e "use finalproject; select round((SUM(case case_status when 'CERTIFIED' then 1 end)/count(case_status))*100,2) as CERTIFIED, round((SUM(case case_status when 'CERTIFIED-WITHDRAWN' 
                     then  1 end)/count(case_status))*100,2) as CERTIFIED_WITHDRAWN, round((SUM(case case_status when 'WITHDRAWN' then 1 end)/count(case_status))*100,2) as WITHDRAWN,
                     round((SUM(case case_status when 'DENIED' then 1 end)/count(case_status))*100,2) as DENIED, year from h1b_final group by year order by year;"
        show_menu;
        ;;
	...
	11) clear;
	    ...
	    case $n in
                ...
                2) 	echo "Part time"
            hive -e "use finalProject;select job_title,full_time_position,year,avg(prevailing_wage) as average from h1b_final where full_time_position ='N' and year='2011' group by job_title,full_time_position,year order by average desc;"
	...
	option_picked "9) Which are   employers who have the highest success rate in petitions more than 70% in petitions and total petions filed more than 1000?"
		pig -x local /home/hduser/pig9.pig
        show_menu;
        ;;
	...
	option_picked "10) Which are the top 10 job positions which have the  success rate more than 70% in petitions and total petitions filed more than 1000?"
		pig -x local /home/hduser/pig10.pig
        ;;
